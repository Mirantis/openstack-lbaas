...
class Writer(object):
    ...
    def writeRServer(self,  rs):
         ...
         cursor = self._con.cursor()
         command = "INSERT INTO rservers (id, sf_id, name, type,webHostRedir,ipType,address,port,state,opstate, description, failOnAll, minCon,  maxCon, weight,  probes, rateBandwidth,   rateConnection,    backupRS,  backupRSport,  created,  updated) ) VALUES('%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s','%s',%d, %d, %d,'%s','%d','%d','%s','%s','%s','%s');"  % (rs.id,  
