...
class AceDriver(BaseDriver):
    ...
    def createRServer(self, context, rserver):
        ...
        if (rserver.type.lower() == "redirect"):
            if self.checkNone(rserver.webHostRedir):
                XMLstr = XMLstr + \
                    "  <webhost-redirection relocation-string='" + \
                    rserver.webHostRedir + "'"
                # without parameter  redirection-code
                XMLstr = XMLstr + " redirection-code='301'"
                XMLstr = XMLstr + "/>\r\n"
    ...
    def activateRServer(self,  context,  serverfarm,  rserver):
        if not self.checkNone(rserver.name) or not \
            self.checkNone(serverfarm.name):
            return 'RSERVER or SERVERFARM NAME ERROR'

        XMLstr = "<serverfarm type='" + serverfarm.type.lower() + \
            "' name='" + serverfarm.name + "'>r\n"
    ...
    def createProbe(self,  context,  probe):
        ...
        probes_with_timeout = ['echo-tcp',  'finger',  'tcp',  'rtsp',  'http',
                                ...
                                'telnet']
        probes_with_credentials = ['http',  'https',  'imap',  'pop',
                                'radius']
        ...
        if (type != 'vm'):
            ...
            if (type == 'rtsp'):
                if self.checkNone(probe.requareHeaderValue):
                    XMLstr = XMLstr + "  <header header-name='Require' \
                        header-value='" + probe.requareHeaderValue + "'/>\r\n"

                if self.checkNone(probe.proxyRequareHeaderValue):
                    XMLstr = XMLstr + "  <header header-name='Proxy-Require' \
            ...
            if self.checkNone(probe.VMControllerName):
                ...
                if (self.checkNone(probe.maxCPUburstThresh) or \
                    ...
                    XMLstr = XMLstr + "  <load type='cpu' \
                        param='burst-threshold'"
                ...
                if (self.checkNone(probe.maxMemBurstThresh) or \
                    ...
                    XMLstr = XMLstr + "  <load type='mem' \
                        param='burst-threshold'"
                    if self.checkNone(probe.maxMemBurstThresh):
                        XMLstr = XMLstr + " max='" + \
                            probe.maxMemBurstThresh + "'"
        ...
        if serverfarm.type.lower() == "host":
            ...
            if self.checkNone(serverfarm.inbandHealthCheck):
                XMLstr = XMLstr + "<inband-health check='" + \
                    serverfarm.inbandHealthCheck.lower() + "'"
    ...
    def deleteServerFarm(self,  context,  serverfarm):
        ...
        XMLstr = "<serverfarm sense='no' type='" + serverfarm.type.lower() + \
            "' name='" + serverfarm.name + "'></serverfarm>"
            ...
            if is_sequence(vip.VLAN):
                for i in vip.VLAN:
                    ...
                    XMLstr = XMLstr + "<service-policy type='input' name='" + \
                        pmap + "'/>\r\n"
                    XMLstr = XMLstr + "<access-group access-type='input' \
                        name='vip-acl'/>\r\n"
    ...
    def deleteVIP(self,  context,  vip):
        ...
        XMLstr = XMLstr + "<class-map sense='no' match-type='match-all' \
            name='" + vip.name + "'>\r\n"
        XMLstr = XMLstr + "</class-map>\r\n"

        XMLstr = XMLstr + "<access-list sense='no' id='vip-acl' \
            config-type='extended' perm-value='permit' "
        XMLstr = XMLstr + "protocol-name='ip' src-type='any' \
            host_dest-addr='" + vip.address + "'/>\r\n"
            ...
            if self.checkNone(vip.allVLANs):
                ...
                if is_sequence(vip.VLAN):
                    for i in vip.VLAN:
                        XMLstr = "<interface type='vlan' number='" + \
                            str(i) + "'>\r\n"
                        XMLstr = XMLstr + "<service-policy sense='no' \
                            type='input' name='" + pmap + "'/>\r\n"
                        XMLstr = XMLstr + "</interface>"
                        tmp = s.deployConfig(context, XMLstr)
