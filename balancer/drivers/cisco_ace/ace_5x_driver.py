    ...
    def createStickiness(self,  context, sticky):
        ...
        if (sticky_type == "http-content"):
            ...
            if self.checkNone(sticky.ipv6PrefixLength):
                cmd += "v6-prefix " + str(sticky.ipv6PrefixLength) + "\n"
        elif sticky_type == "v6prefix":
            cmd += str(sticky.prefixLength) + " address " + \
                sticky.addrType.lower() + " " + name + "\n"
            if self.checkNone(sticky.netmask):
                cmd += "ip-netmask " + str(sticky.netmask) + "\n"
