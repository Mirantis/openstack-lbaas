...
logger = logging.getLogger('balancer.api.v1.sessionPersistence')


class Controller(object):

    def __init__(self, conf):
        logger.debug("Creating sessionPersistence controller with config:"
                                                "sessionPersistence.py %s", conf)
        self.conf = conf
